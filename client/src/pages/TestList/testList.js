        ...
        <Row className="form-group">>
          <Col size="12">
            <table className="table">
              ...
              <tbody>
                {currentTests && user && (currentTests.map((test, index) => {
                  let foundRecord = checkTestRecord(test);
                  return (
                    <tr key={test._id + "1"}>
                      <th scope="row" key={test._id + "2"}>{test.testName}</th>
                      <td key={test._id + "3"}>1/1/21</td>
                      <td key={test._id + "4"}>{foundRecord ? "Complete" : "Incomplete"}</td>
                      <td key={test._id + "5"}>
                        {foundRecord ?
                          ...
                          <button key={test._id + "btn2"} className="btn btn-primary">
                            <Link key={test._id+"link2"} to={{
                              pathname:"/test",
                              state:{"testid": test._id}
                              }} className="btnLink">Start</Link>
                          </button>
                        }
                      </td>
                    </tr>
